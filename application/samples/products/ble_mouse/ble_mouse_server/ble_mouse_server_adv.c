...
static uint8_t ble_set_adv_appearance(uint8_t *set_adv_data_position, uint8_t max_len)
{
    ...
    ble_appearance_t adv_appearance_data = {
        ...
        .catogory_id = { u16_low_u8(BLE_APPEARANCE_MOUSE), u16_high_u8(BLE_APPEARANCE_MOUSE)},
    };
    n_ret = memcpy_s(set_adv_data_position, max_len, &adv_appearance_data, sizeof(adv_appearance_data));
    if (n_ret != EOK) {
        return 0;
    }

    return sizeof(adv_appearance_data);
}
