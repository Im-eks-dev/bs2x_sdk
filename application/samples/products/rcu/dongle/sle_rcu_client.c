...
static sle_addr_t *g_client_addr = NULL;
#endif
...
#if defined (CONFIG_RCU_MASS_PRODUCTION_TEST)
static void sle_rcu_record_client_addr(sle_addr_t *addr)
{
    g_client_addr = addr;
    return;
}
...
{
    return g_client_addr;
}

static void sle_rcu_client_sample_auth_complete_cbk(uint16_t conn_id, const sle_addr_t *addr, errcode_t status,
    const sle_auth_info_evt_t* evt)
{
    ...
    unused(status);
    unused(evt);
    sle_addr_t *own_addr = NULL;
    sle_get_local_addr(own_addr);
    sle_set_nv_smp_keys((sle_auth_info_evt_t *)evt, own_addr, (sle_addr_t *)addr);
    sle_rcu_record_client_addr((sle_addr_t *)addr);
}
#endif
...
#if defined (CONFIG_RCU_MASS_PRODUCTION_TEST)
    g_sle_rcu_connect_cbk.auth_complete_cb = sle_rcu_client_sample_auth_complete_cbk;
#endif
