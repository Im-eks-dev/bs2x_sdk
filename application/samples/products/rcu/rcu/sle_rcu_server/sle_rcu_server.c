...
/* 低功耗连接参数信息 */
static sle_connection_param_update_t g_work_to_standby = { 0, 60, 90, 30, 3000 };
static sle_connection_param_update_t g_standby_to_work = { 0, 90, 90, 0, 3000 };
...
uint8_t g_encode_data_len = 0;
uint8_t g_out_encode_data_valid = 0;
...
static errcode_t sle_sample_set_uuid(uint8_t *uuid, sle_uuid_t *service_uuid)
{
    if (memcpy_s(service_uuid->uuid, SLE_UUID_LEN, uuid, SLE_UUID_LEN) != EOK) {
        sample_print("sle mouse hid set uuid fail\r\n");
...
static errcode_t sle_remote_control_service_add(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[uuid server] sle mouse uuid set fail, ret:%x\r\n", ret);
        return ERRCODE_SLE_FAIL;
    }

    ret = ssaps_add_service_sync(g_server_id, &service_uuid, 1, &g_service_hdl[SLE_UUID_INDEX0].handle_out);
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[uuid server] sle mouse add service fail, ret:%x\r\n", ret);
        return ERRCODE_SLE_FAIL;
    }
...
static errcode_t sle_remote_control_property_and_descriptor_add(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[uuid server] sle mouse add report fail, ret:%x, indet:%x\r\n", ret, SLE_UUID_INDEX1);
        return ERRCODE_SLE_FAIL;
    }
    sample_print("[uuid server] sle mouse add report, proterty hdl:%x\r\n",
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[uuid server] sle mouse add cccd fail, ret:%x, indet:%x\r\n", ret, SLE_UUID_INDEX2);
        return ERRCODE_SLE_FAIL;
    }
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[uuid server] sle mouse add report ref fail, ret:%x, indet:%x\r\n", ret, SLE_UUID_INDEX3);
        return ERRCODE_SLE_FAIL;
    }
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[uuid server] sle mouse add report map ref fail, ret:%x, indet:%x\r\n", ret,
            SLE_UUID_INDEX4);
        return ERRCODE_SLE_FAIL;
    }
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[uuid server] sle mouse add hid ctrl point fail, ret:%x, indet:%x\r\n", ret,
            SLE_UUID_INDEX5);
        return ERRCODE_SLE_FAIL;
    }
    return ERRCODE_SLE_SUCCESS;
...
errcode_t sle_rcu_server_add(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("[1]%s sle rcu add service fail, ret:%x\r\n", SLE_RCU_SERVER_LOG, ret);
        return ERRCODE_SLE_FAIL;
    }
    // add HID service
    ...
    sample_print("[2]%s sle rcu add service, server_id:%x, service_handle:%x, property_handle:%x\r\n",
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle rcu add HID service fail, ret:%x\r\n", SLE_RCU_SERVER_LOG, ret);
        return ERRCODE_SLE_FAIL;
    }
...
{
    if (memcpy_s(g_out_low_latency_data, LOW_LATENCY_DATA_MAX, data1, data1_len) != EOK) {
        osal_printk("memcpy first part data fail.\r\n");
    }
    if (memcpy_s(g_out_low_latency_data + data1_len, LOW_LATENCY_DATA_MAX - data1_len, data2,
        data2_len) != EOK) {
        osal_printk("memcpy second part data fail.\r\n");
    }

    g_encode_data_len = data1_len + data2_len;
    g_out_encode_data_valid = 1;
}
...
uint8_t *sle_low_latency_get_data_cbk(uint8_t *length, uint16_t *ssap_handle, uint8_t *data_type, uint16_t co_handle)
{
    ...
    *ssap_handle = SLE_RCU_SSAP_RPT_HANDLE;
    if (g_out_encode_data_valid == 1) {
        g_out_encode_data_valid = 0;
        *length = g_encode_data_len;
...
void sle_set_em_data_cbk(uint16_t co_handle, uint8_t status)
{
    ...
    sample_print("pair sle_set_em_data_cbk\r\n");
}
...
static void sle_pair_complete_cbk(uint16_t conn_id, const sle_addr_t *addr, errcode_t status)
{
    ...
    sle_low_latency_set_em_data(conn_id, 1);
}
