/**
 * Copyright (c) @CompanyNameMagicTag 2023-2024. All rights reserved. \n
 *
 * Description: Sle Mouse BAS Server Inc. \n
 * Author:  @CompanyNameMagicTag  \n
 * History: \n
 * 2024-04-06, Create file. \n
 */

#ifndef SLE_MOUSE_BAS_SERVER_H
#define SLE_MOUSE_BAS_SERVER_H

#include "sle_ssap_server.h"
#include "service_common.h"
#include "bas_server.h"

/**
 * @if Eng
 * @brief  Initializing the SLE BAS Service Instance.
 * @param [in] server_id server id.
 * @attention  NULL
 * @retval ERRCODE_SLE_SUCCESS    Excute successfully
 * @retval ERRCODE_SLE_FAIL       Execute fail
 * @else
 * @brief  SLE BAS服务实例初始化。
 * @param [in] server_id 服务ID。
 * @attention  NULL
 * @retval ERRCODE_SLE_SUCCESS    执行成功
 * @retval ERRCODE_SLE_FAIL       执行失败
 * @endif
 */
errcode_t sle_mouse_bas_server_init(uint8_t server_id);

/**
 * @if Eng
 * @brief  Get the input report attribute handle of the SLE BAS service instance..
 * @attention  NULL
 * @retval input report handle.
 * @else
 * @brief  SLE BAS服务剩余变量百分比数据发送。
 * @attention  NULL
 * @retval 输入报告句柄。
 * @endif
 */
void sle_bas_send_remain_capacity_ratio(uint8_t server_id, uint16_t conn_id, uint8_t remain_capacity_ratio);

#endif /* END of SLE_MOUSE_BAS_SERVER_H */
