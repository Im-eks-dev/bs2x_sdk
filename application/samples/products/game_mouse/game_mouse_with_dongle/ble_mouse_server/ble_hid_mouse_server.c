...
#define BLE_CBK_PRINT_INTERVAL 100 // 500ms日志打印间隔
...
static uint8_t g_device_mouse_name_value[32] = "Turnkey_mouse";
static uint8_t g_device_mouse_name_len = 14;
...
#define HID_ADD_CHARACTER_DESCRIPTOR_USE_ASYNCHRONOUS   0
static bt_uuid_t hid_service_uuid = { 0 };
...
static void ble_hid_add_service(void)
{
    ...
    gatts_add_service(g_server_id, &hid_service_uuid, true);
}
...
#if HID_ADD_CHARACTER_DESCRIPTOR_USE_ASYNCHRONOUS
    gatts_add_characteristic(server_id, srvc_handle, &character);
#else
    gatts_add_character_result_t character_reslut;
    gatts_add_characteristic_sync(server_id, srvc_handle, &character, &character_reslut);
    osal_printk("\n");
    osal_printk("ble_hid_add_character_hid_information:0x%02x,0x%02x,\n", character_reslut.handle,
        character_reslut.value_handle);
#endif
}
...
static void ble_hid_add_character_report_map(uint8_t server_id, uint16_t srvc_handle)
{
    ...
    gatts_add_characteristic_sync(server_id, srvc_handle, &character, &character_reslut);
    osal_printk("\n");
    osal_printk("ble_hid_add_character_report_map:0x%04x,0x%04x,\n", character_reslut.handle,
        character_reslut.value_handle);
#endif
}
...
static void ble_hid_add_character_hid_control_point(uint8_t server_id, uint16_t srvc_handle)
{
    ...
    gatts_add_characteristic_sync(server_id, srvc_handle, &character, &character_reslut);
    osal_printk("\n");
    osal_printk("ble_hid_add_character_hid_control_point:0x%04x,0x%04x,\n", character_reslut.handle,
        character_reslut.value_handle);
#endif
}
...
static void ble_hid_add_character_input_report(uint8_t server_id, uint16_t srvc_handle)
{
    ...
    gatts_add_character_result_t character_reslut;
    gatts_add_characteristic_sync(server_id, srvc_handle, &character, &character_reslut);
    if (g_hid_input_report_att_hdl == INVALID_ATT_HDL) {
        g_hid_input_report_att_hdl = character_reslut.value_handle;
    }
#endif
...
errcode_t ble_hid_server_register_gatt_callbacks(void)
{
...
errcode_t ble_hiddev_mouse_server_send_input_report(const ble_hid_high_mouse_event_st *mouse_data)
{
    ...
    param.value = (uint8_t *)&mouse_data->button_mask;
...
errcode_t ble_hiddev_mouse_server_send_input_report_by_uuid(const ble_hid_high_mouse_event_st *mouse_data)
{
    ...
    param.value_len = sizeof(ble_hid_high_mouse_event_st);
    param.value =(uint8_t *)&mouse_data->button_mask;
    gatts_notify_indicate_by_uuid(g_server_id, BLE_SINGLE_LINK_CONNECT_ID, &param);
    return ERRCODE_BT_SUCCESS;
}

uint16_t ble_get_hid_report_att_hdl(void)
{
    return g_hid_input_report_att_hdl;
}
