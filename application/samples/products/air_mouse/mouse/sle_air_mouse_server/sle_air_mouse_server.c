...
#define SLE_AIR_MOUSE_SSAP_RPT_HANDLE       4 // SLP数传HANDLE
#define SLE_AIR_MOUSE_CURSOR_RPT_HANDLE     6 // 鼠标坐标上报HANDLE
#define SLE_AIR_MOUSE_KEY_RPT_HANDLE        8 // 鼠标按键上报HANDLE
#define SLE_AIR_MOUSE_KEYBOARD_RPT_HANDLE   10 // 键盘按键上报HANDLE
...
static void check_into_sleep(uint16_t x, uint16_t y)
{
...
void cursor_report_cbk(uint16_t x, uint16_t y)
{
    // 如果需要睡眠，此处调用
    if (CONFIG_LOW_POWER_MODE == 1) {
        check_into_sleep(x, y);
    }
    sle_hid_mouse_server_send_cursor_report((int16_t)x, (int16_t)y);
}

void register_slp_report_cursor_callback(void)
{
    SlpReportCallbacks cbks = {0};
    cbks.rptCursorCbk = cursor_report_cbk;
    if (SlpRegisterReportCallbacks(&cbks) != ERRCODE_SLPC_SUCCESS) {
        osal_printk("register slp_report_cursor_callback failed\r\n");
    }
}
...
errcode_t sle_air_mouse_server_init(void)
{
    ...
    register_slp_report_cursor_callback(); // 注册坐标上报回调函数
