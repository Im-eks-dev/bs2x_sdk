...
void sle_keyboard_start_scan(void)
{
    ...
    sle_start_seek();
    osal_msleep(SLE_KEYBOARD_TASK_DELAY_MS);
}
...
static void sle_keyboard_client_sample_ssapc_cbk_register(void)
{
    ...
    g_sle_keyboard_ssapc_cbk.notification_cb = sle_keyboard_notification_cb;
    g_sle_keyboard_ssapc_cbk.indication_cb = sle_keyboard_indication_cb;
    ssapc_register_callbacks(&g_sle_keyboard_ssapc_cbk);
}

void sle_power_on_cb(uint8_t status)
{
    osal_printk("sle power on: %d.\r\n", status);
    enable_sle();
}

void sle_enable_cb(uint8_t status)
{
    osal_printk("sle enable: %d.\r\n", status);
    sle_keyboard_client_sample_seek_cbk_register();
    sle_keyboard_client_sample_connect_cbk_register();
    sle_keyboard_client_sample_ssapc_cbk_register();
    sle_keyboard_start_scan();
}
...
void sle_enable_cbk_register(void)
{
    ...
    dev_cb.sle_power_on_cb = sle_power_on_cb;
    dev_cb.sle_enable_cb = sle_enable_cb;
    ret = sle_dev_manager_register_callbacks(&dev_cb);
