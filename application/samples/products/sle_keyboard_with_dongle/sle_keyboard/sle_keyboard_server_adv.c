...
static bool g_sle_keyboard_inited = false;
...
static int sle_set_default_announce_data(void)
{
    ...
    sample_print("%s sizeof(g_sle_adv_data) = %d\r\n", SLE_KEYBOARD_SERVER_LOG, sizeof(g_sle_adv_data));
...
{
    sample_print("sle power on: %d.\r\n", status);
    if (!g_sle_keyboard_inited && enable_sle() == ERRCODE_SLE_SUCCESS) {
        g_sle_keyboard_inited = true;
        sample_print("sle enable success\r\n");
        return;
    }
}
