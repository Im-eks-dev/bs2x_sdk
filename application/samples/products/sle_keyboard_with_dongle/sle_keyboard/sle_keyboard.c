...
#define SLE_KEYBOARD_TASK_PRIO                  24
                                           ...
                                           errcode_t status)
{
    ...
    if ((write_cb_para->length > 0) && write_cb_para->value) {
        ...
        // key_nums > 0，需调用uapi_ble_hid_keyboard_input_str(key_nums, (char **)key_value_str)或者其他接口将消息发送到键盘
...
static int sle_keyboard_keyscan_callback(int key_nums, uint8_t key_values[])
{
    ...
    osal_printk("sle_keyboard_keyscan_callback key_nums:%d\r\n", key_nums);
    for (int i = 0; i < key_nums; i++) {
        osal_printk("key_values[%d]:%d\r\n", i, key_nums);
    }
    ...
    sle_hid_keyboard_server_send_report_by_handle((uint8_t *)(uintptr_t)&g_hid_keyboard_report,
...
static void *sle_keyboard_task(const char *arg)
{
    ...
    osal_msleep(SLE_KEYBOARD_TASK_DURATION_MS * SLE_KEYBOARD_SERVER_DELAY_COUNT);
    ...
    osal_msleep(SLE_KEYBOARD_TASK_DURATION_MS);
...
static void sle_keyboard_entry(void)
{
    ...
    osal_kthread_lock();
    task_handle = osal_kthread_create((osal_kthread_handler)sle_keyboard_task, 0, "SLEKeyboardServerTask",
                                      SLE_KEYBOARD_TASK_STACK_SIZE);
    if (task_handle != NULL) {
        osal_kthread_set_priority(task_handle, SLE_KEYBOARD_TASK_PRIO);
    }
    osal_kthread_unlock();
}
