...
static void app_spi_trx_init_pin(void)
{
    ...
    uapi_pin_set_ie(CONFIG_SPI_TRX_SLAVE_CLK_PIN, PIN_IE_1);
    uapi_pin_set_ie(CONFIG_SPI_TRX_SLAVE_CS_PIN, PIN_IE_1);
    uapi_pin_set_ie(CONFIG_SPI_TRX_SLAVE_DI_PIN, PIN_IE_1);
#endif /* CONFIG_PINCTRL_SUPPORT_IE */
    uapi_pin_set_mode(CONFIG_SPI_TRX_SLAVE_CLK_PIN, CONFIG_SPI_TRX_SLAVE_CLK_PIN_MODE);
    uapi_pin_set_mode(CONFIG_SPI_TRX_SLAVE_CS_PIN, CONFIG_SPI_TRX_SLAVE_CS_PIN_MODE);
    uapi_pin_set_mode(CONFIG_SPI_TRX_SLAVE_DI_PIN, CONFIG_SPI_TRX_SLAVE_DI_PIN_MODE);
    uapi_pin_set_mode(CONFIG_SPI_TRX_SLAVE_DO_PIN, CONFIG_SPI_TRX_SLAVE_DO_PIN_MODE);
}

static void app_spi_trx_slave_init_config(void)
{
    spi_attr_t config = { 0 };
    ...
    uapi_spi_init(CONFIG_SPI_TRX_SLAVE_BUS_ID, &config, &ext_config);
}

static void *spi_trx_slave_task(const char *arg)
{
    unused(arg);
    /* SPI pinmux. */
    ...
    while (g_spi_test_times--) {
        uapi_spi_slave_write(CONFIG_SPI_TRX_SLAVE_BUS_ID, &data, 0xFFFFFFFF);
        if (uapi_spi_slave_read(CONFIG_SPI_TRX_SLAVE_BUS_ID, &data, 0xFFFFFFFF) == ERRCODE_SUCC) {
    ...
    while (1) {
        ...
        osal_printk("spi%d trx slave receive start!\r\n", CONFIG_SPI_TRX_SLAVE_BUS_ID);
        if (uapi_spi_slave_read(CONFIG_SPI_TRX_SLAVE_BUS_ID, &data, 0xFFFFFFFF) == ERRCODE_SUCC) {
            for (uint32_t i = 0; i < data.rx_bytes; i++) {
                osal_printk("spi%d trx slave receive data is %x\r\n", CONFIG_SPI_TRX_SLAVE_BUS_ID, data.rx_buff[i]);
            }
            osal_printk("spi%d trx slave receive succ!\r\n", CONFIG_SPI_TRX_SLAVE_BUS_ID);
        } else {
            continue;
        }
        osal_printk("spi%d trx slave send start!\r\n", CONFIG_SPI_TRX_SLAVE_BUS_ID);
        if (uapi_spi_slave_write(CONFIG_SPI_TRX_SLAVE_BUS_ID, &data, 0xFFFFFFFF) == ERRCODE_SUCC) {
            osal_printk("spi%d trx slave send succ!\r\n", CONFIG_SPI_TRX_SLAVE_BUS_ID);
        }
    }
#endif
    return NULL;
...
static void spi_trx_slave_entry(void)
{
    osal_task *task_handle = NULL;
    osal_kthread_lock();
    task_handle = osal_kthread_create((osal_kthread_handler)spi_trx_slave_task, 0, "SpiTrxSlaveTask",
...
/* Run the spi_trx_slave_entry. */
app_run(spi_trx_slave_entry);