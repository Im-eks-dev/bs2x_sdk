...
/**
 * @if Eng
 * @brief  Transmit and simultaneously receive interface for hal SPI.
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t
 * @param  [out] data Transfer data. For details, see @ref hal_spi_xfer_data_t
 * @param  [in]  timeout Timeout duration.
 * @retval ERRCODE_SUCC Success.
 ...
 * @endif
 */
errcode_t hal_spi_txrx_transfer(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout);
#endif
