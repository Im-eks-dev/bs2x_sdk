...
typedef enum hal_uart_ctrl_id {
    ...
    UART_CTRL_EN_TX_INT,           /*!< @if Eng Enable the tx interrupt.
                                        @else   使能TX中断 @endif */
#endif
    UART_CTRL_EN_RX_INT,           /*!< @if Eng Enable the RX interrupt.
                                        @else   使能RX中断 @endif */
    UART_CTRL_EN_IDLE_INT,         /*!< @if Eng Enable the RX IDLE interrupt.
                                        @else   使能接收IDLE中断 @endif */
    UART_CTRL_EN_PARITY_ERR_INT,   /*!< @if Eng Enable the parity error interrupt.
                                        @else   使能奇偶校验错误中断 @endif */
    UART_CTRL_EN_FRAME_ERR_INT,    /*!< @if Eng Enable the frame errorinterrupt.
                                        @else   使能帧错误中断 @endif */
    UART_CTRL_EN_OVERRUN_ERR_INT,  /*!< @if Eng Enable the overrun errorinterrupt.
                                        @else   使能溢出错误中断 @endif */
