...
/** Rx sample baud rate */
typedef enum {
    HAL_UART_RX_SAMPLE_PHASE_EIGHT = 0x3,
    HAL_UART_RX_SAMPLE_PHASE_SIXTEEN = 0x7,
} hal_rx_sample_phase_t;
 ...
 * @param  uart Uart bus.
 * @param  sample_baud_rate Sample baud rate.
 * @param  rx_sample_phase Rx sample phase.
 */
void hal_uart_set_baud_div(uart_bus_t uart, uint8_t sample_baud_rate, uint8_t rx_sample_phase);
...
/**
 * @brief  Enable/disable uart uniform division.
 * @param  uart The uart needs to be enable/disable uart low power request.
 * @param  val True means enable uart uniform division, false means disable.
 */
void hal_uart_set_uniform_div_en(uart_bus_t uart, bool val);
#endif
