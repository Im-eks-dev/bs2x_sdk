...
typedef union fifo_ctl {
    ...
    struct {
        ...
        uint32_t rx_empty_trig_l : 3; /* !< WO, RX Empty LOW Trigger
                                             ...
                                             101:111 = reserved. */
        uint32_t rx_empty_trig_h : 3; /* !< WO, RX Empty HIGH Trigger
                                             000 = Receive FIFO becomes >= 1/8 full
                                             001 = Receive FIFO becomes >= 1/4 full
                                             010 = Receive FIFO becomes >= 1/2 full
                                             011 = Receive FIFO becomes >= 3/4 full
...
typedef union waterline_ctl {
    uint32_t d32;
    struct {
        uint32_t double_wl_enable : 1;
    } waterline_ctl;
} waterline_ctl_t;

typedef union uniform_div_ctl {
    uint32_t d32;
    struct {
        uint32_t uniform_div_enable : 1;
    } uniform_div_ctl;
} uniform_div_ctl_t;

typedef union rts_release_control {
    uint32_t d32;
    struct {
        uint32_t rts_release_sel : 1;
    } rts_release_control;
} rts_release_control_t;
...
typedef struct uart_regs {
    ...
    volatile uint32_t waterline_ctl;        /*!< Enable Double Waterline.  <i>Offset: 6Ch</i>. */
    volatile uint32_t uniform_div_ctl;      /*!< Uniform Division Control.  <i>Offset: 70h</i>. */
    volatile uint32_t rts_release_control;  /*!< RTS Release Control.  <i>Offset: 74h</i>. */
#endif
} uart_reg_t;
...
typedef struct uart_regs1 {
    ...
    volatile waterline_ctl_t waterline_ctl;     /*!< Enable Double Waterline.  <i>Offset: 6Ch</i>. */
    volatile uniform_div_ctl_t uniform_div_ctl; /*!< Uniform Division Control.  <i>Offset: 70h</i>. */
    volatile rts_release_control_t rts_release_control; /*!< RTS Release Control.  <i>Offset: 74h</i>. */
#endif
} uart_reg_1_t;
