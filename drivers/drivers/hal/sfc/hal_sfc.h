 ...
 * @param  [in]  mapping   Flash mapping address.
...
/**
 * @if Eng
 * @brief  SFC wait last operations done.
 * @param  [in]  wip_bit  wait last operations done bit offset.
 * @retval ERRCODE_SUCC   Success.
 * @retval Other          Failure. For details, see @ref errcode_t.
 * @else
 * @brief  等待上一次访问动作完成。
 * @param  [in]  wip_bit 等待完成的bit位移位数。
 * @retval ERRCODE_SUCC 成功。
 * @retval Other        失败，参考 @ref errcode_t 。
 * @endif
 */
