...
/**
 * @if Eng
 * @brief  Suspend pin config for hal pin.
 * @param  [in] pin Definition of IO. see @ref pin_t
 * @retval ERRCODE_SUCC Success.
 * @retval Other        Failure. For details, see @ref errcode_t
 * @else
 ...
 * @endif
 */
typedef errcode_t (*hal_pin_suspend)(pin_t pin);

/**
 * @if Eng
 * @brief  Resume pin config for hal pin.
 * @param  [in] pin Definition of IO. see @ref pin_t
 * @return Schmitt-trigger status. see @ref pin_schmitt_trigger_t
 * @else
 * @brief  HAL层恢复pin配置。
 ...
 * @endif
 */
typedef errcode_t (*hal_pin_resume)(pin_t pin);
...
#endif /* CONFIG_PINCTRL_SUPPORT_ST */
#if defined(CONFIG_PINCTRL_SUPPORT_LPM)
    hal_pin_suspend  suspend;        /*!< @if Eng suspend pin config interface.
                                         @else   保存pin配置接口。 @endif */
    hal_pin_resume   resume;        /*!< @if Eng resume pin config interface.
                                         @else   恢复pin配置接口。 @endif */
#endif /* CONFIG_PINCTRL_SUPPORT_LPM */
} hal_pin_funcs_t;
