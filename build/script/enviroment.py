...
class TargetEnvironment:
    def __init__(self, target_name, extra_defines=None, extra_cclfgas=None):
        ...
        if "sector_cfg" in self.config:
            cfg_file = os.path.join(target_config_path, self.config['chip'], 'flash_sector_config', f"{self.config['sector_cfg']}.json")
            if os.path.exists(cfg_file):
                with open(cfg_file, 'r') as cfg_json:
                    load_cfg_json = json.load(cfg_json)
                    if ("Partition_defines" in load_cfg_json and "Use_defines" in load_cfg_json and (load_cfg_json['Use_defines'])):
                        param_defines = load_cfg_json['Partition_defines']
                        param_info = load_cfg_json['Partition_Tbl']['param_info']
                        error = 0 #错误标识，方便打印出所有错误
                        for id_defines in param_defines:
                            for id_info in param_info:
                                if id_info[0] == id_defines[0]:
                                    self.append("defines", "{}={}".format(id_defines[1], id_info[1]))
                                    self.append("defines", "{}={}".format(id_defines[2], id_info[2]))
                        new_param_info = [] #生成新列表
                        id_set = {elem[0] for elem in param_defines}
                        for element in param_info:
                            id_value = element[0]
                            length = element[2]
                            if id_value in id_set and int(length, 16) != 0:
                                new_element = (id_value, int(element[1], 16), int(element[2], 16))
                                new_param_info.append(new_element)
                        for i in range(len(new_param_info)):
                            for j in range(i + 1, len(new_param_info)):
                                info_i = new_param_info[i]
                                info_j = new_param_info[j]
                                start_i = info_i[1]
                                end_i = info_i[1] + info_i[2]
                                start_j = info_j[1]
                                end_j = info_j[1] + info_j[2]
                                if (start_i < end_j and end_i > start_j):
                                    print(f"[ERROR]:分区重叠：{cfg_file}中的{info_i[0]}和{info_j[0]}")
                                    error = 1
                        if (error != 0):
                            for i in param_info:
                                print(i)
                        ...
                        data = cfg_json.read()
                        m = re.search("\"[0x]+\", *\"[0-9A-Za-z]+\", *\"([0-9A-Za-z]+)\"", data)
                        if m:
                            self.append("defines", f"FLASHBOOT_A_IMAGE_LEN={m.group(1)}")
                        m = re.search("\"[0x]*1\", *\"[0-9A-Za-z]+\", *\"([0-9A-Za-z]+)\"", data)
                        if m:
                            self.append("defines", f"FLASHBOOT_B_IMAGE_LEN={m.group(1)}")
