...
macro(replace_lib_output_path)
    ...
    if(DEFINED CONFIG_BUILD_REDIRECTS_STATIC_LIBRARY_PATH)
        set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY "${ROOT_DIR}/library/${TARGET_COMMAND}")
    endif()
endmacro()
...
macro(build_component)
    ...
    # 排除目录列表，目前只排除 `${ROOT_DIR}/application`
    set(EXCLUDE_DIRS ${ROOT_DIR}/application)
    get_filename_component(CURRENT_ABS_DIR ${CMAKE_CURRENT_SOURCE_DIR} ABSOLUTE)
    set(IS_EXCLUDED FALSE)
    foreach(EXCLUDE_DIR ${EXCLUDE_DIRS})
        get_filename_component(EXCLUDE_ABS_DIR ${EXCLUDE_DIR} ABSOLUTE)
        # 判断当前目录是否以排除路径开头（即子目录）
        string(FIND "${CURRENT_ABS_DIR}" "${EXCLUDE_ABS_DIR}" POSITION)
        if(POSITION EQUAL 0)
            set(IS_EXCLUDED TRUE)
            break()
        endif()
    endforeach()
    if(NOT IS_EXCLUDED AND DEFINED CONFIG_BUILD_USING_STATIC_LIBRARIES_SDK)
